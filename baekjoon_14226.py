'''
Problem Solving Baekjoon 14226
Author: Injun Son
...
S = int(input())
visited = [sys.maxsize]*(2001)
visited[1]=0

def bfs(v, time, clip_count):
    #이모티콘의 v는 개수이다
    visited[v] = 1
    q = deque()
    q.append([v, time, clip_count])
    while q:
        v, time, clip_count = q.popleft()
        if v==S:
            visited[v] = time
            return
        else:
            #화면에 있는 이모티콘 모두 복사,  클립보드 저장, AND 붙여넣기
            if v*2<=2000 and visited[v]+2 <=visited[v*2]:
                visited[v*2] = time+2
                q.append([v*2, time+2, v])

            #화면에 있는 이모티콘 중 하나 삭제
            if 0<= v-1 and visited[v]+1 <= visited[v-1]:
                visited[v-1] = time+1
                q.append([v-1, time+1, clip_count])

            #화면에 있는 것 복사안하고 그냥 클립보드에 있는것 붙여넣기
            if v+clip_count <=2000 and visited[v]+1 <= visited[v+clip_count]:
                visited[v+clip_count] = visited[v]+1
                q.append([v+clip_count, time+1, clip_count])
